...
import { IRapor, RaporFormValues } from "@/types/Rapor";
import raporServices from "@/services/rapor.service";
import { ToasterContext } from "@/contexts/ToasterContext";
import useRapor from "../useRapor";
...
const useAddRapor = () => {
  ...
  const handleAddRapor = (data: RaporFormValues) => {
    const payload: IRapor & { caberawitId: number } = {
      caberawitId: Number(id),
      semester: data.semester,
      raporItems: (data.raporItems ?? []).map((item) => ({
